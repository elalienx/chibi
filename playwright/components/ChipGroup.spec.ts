...
test.beforeEach(async ({ mount }) => {
  form = await mount("forms/example-chip-group/FormPage/Default");
});
...
test("2. Should submit form without errors (boolean is TRUE)", async () => {
  ...
  await expect(form.getByText("Chip 2: TRUE")).toBeVisible();
});
...
test("3. Should submit form without errors (boolean is FALSE)", async () => {
  ...
  await expect(form.locator(error1)).not.toBeVisible();
  await expect(form.locator(error2)).not.toBeVisible();
  await expect(form.getByText("Chip 2: FALSE")).toBeVisible();
});

test("4. Clicking on a chip option with error should immediately remove the error", async () => {
