...
test("Should be able to submit with no debt", async ({ mount, page }) => {
  await mount("forms/mvp-business/FormManager/Default");
  ...
  await fillStep4(page, { turnover: 1_000_000, hasExistingLoans: false });

  await checkSuccessStep(page, { result1 });
});
...
test("Should be able to submit with debt", async ({ mount, page }) => {
  ...
  await fillStep4(page, { turnover: 500_000, hasExistingLoans: true, loanDebt: 250_000 });
